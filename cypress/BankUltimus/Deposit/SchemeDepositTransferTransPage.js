class SchemeDepositTrfTrans {

    SDtrfTrans() {


        const dataSDtrfTrans = Cypress.env('excelData');
        cy.intercept('POST', '/http://192.168.20.127/BankUltimus/src/BankUltimus.UI/BU_Trans/CorTRFTransSDUI.aspx?FUNCTION_ID=0125009&FAST_PATH=7032').as('formReload');
    ...
    //     // cy.get('#ctl00_contPlcHdrMasterHolder_LsddlDrCr').focus().select(dataDDtrfTrans.DrCr);
        ...
        cy.get('#ctl00_contPlcHdrMasterHolder_LstxTransAmtCcy').type(dataSDtrfTrans.amount).type('{enter}');
 

        cy.wait(3000);
        cy.get('#ctl00_contPlcHdrMasterHolder_LstxtNarration').clear();
        cy.get('#ctl00_contPlcHdrMasterHolder_LstxtNarration').type(dataSDtrfTrans.narration).type('{enter}');
...
export default SchemeDepositTrfTrans;